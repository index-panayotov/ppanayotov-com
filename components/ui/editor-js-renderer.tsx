...
const sanitize = (html: string): string => {
  return DOMPurify.sanitize(html, {
    ALLOWED_TAGS: ['b', 'i', 'em', 'strong', 'a', 'code', 'mark', 'u', 's', 'sub', 'sup', 'br'],
      ...
      {blocks.map((block) => {
        switch (block.type) {
          ...
          case 'header':
            ...
            return <HeaderTag key={block.id} dangerouslySetInnerHTML={{ __html: sanitize(block.data.text) }} />;
          case 'list':
            const ListTag = block.data.style === 'ordered' ? 'ol' : 'ul';
            return (
              <ListTag key={block.id}>
                {block.data.items.map((item: string, index: number) => (
                  <li key={index} dangerouslySetInnerHTML={{ __html: sanitize(item) }} />
                ))}
              </ListTag>
            );
          case 'quote':
            return (
              <blockquote key={block.id}>
                ...
                {block.data.caption && <footer dangerouslySetInnerHTML={{ __html: sanitize(block.data.caption) }} />}
              </blockquote>
            );
          case 'code':
            return (
